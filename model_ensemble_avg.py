...
def ensemble(train_data=True):
    ...
    while True:
        ...
        for i in range(len(ensembles)):
            print ' [%d] %s weight %f' % (i, ensembles[i][0][0], ensembles[i][1])
            final_result = final_result + (ensembles[i][0][1] * ensembles[i][1])
            weight += ensembles[i][1]

        if len(ensembles) > 0:
            final_result = final_result / weight
        print '-----------------------------'
        ...
        if cmd == 'a' or cmd =='A':
            ...
            if idx < len(files) and idx >= 0:
                ensembles.append((files[idx], weight))
        elif cmd == 'd' or cmd =='D':
            idx = int(raw_input('number>'))
